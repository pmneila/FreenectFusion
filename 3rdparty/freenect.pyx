...
cdef extern from "libfreenect_sync.h":
    ...
    int freenect_sync_set_led(int led, int index)
...
def sync_set_led(int led, int index=0):
    return freenect_sync_set_led(led, index)

def sync_set_tilt_degs(int angle, int index=0):
